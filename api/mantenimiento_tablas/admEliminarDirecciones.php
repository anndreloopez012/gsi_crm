...
if (isset($_GET["validaciones"]) && !empty($_GET["validaciones"])) {
    ...
    if ($strTipoValidacion == "delete") {
        ...
        $var_consulta = "DELETE FROM DIRECCIONES WHERE NIU = $id_data;";
        ...
        if (!empty($buscarNOMBRE)) {
            $strFilterNOMBRE = " AND ( UPPER(direccion) LIKE UPPER('%{$buscarNOMBRE}%') ) ";
        }
        ...
        $arrData = array();
        $var_consulta = "SELECT NIU,CODICLIE,DIRECCION  
        FROM DIRECCIONES
        WHERE 1=1
        $strFilterNOMBRE
        ...
        while ($rTMP = pg_fetch_assoc($var_consulta)) {
            ...
            $arrData[$rTMP["niu"]]["DIRECCION"]             = $rTMP["direccion"];
