...
if (isset($_GET["validaciones"]) && !empty($_GET["validaciones"])) {
    ...
    if ($strTipoValidacion == "insert") {
        ...
        foreach ($contactList as $key => $contactData) {
            ...
            $rsNIU = pg_query("SELECT COUNT(NIU) FROM DIRECCIONES WHERE CODICLIE = '{$contactData[0]}' AND DIRECCION = '{$contactData[1]}'");
            if ($row = pg_fetch_row($rsNIU)) {
                $idRow = trim($row[0]);
            }
            ...
            if ($numeroExiste == 0) {
                $var_consulta = "INSERT INTO DIRECCIONES ( CODICLIE, DIRECCION) VALUES ( '$contactData[0]', '{$contactData[1]}');";
