...
if (isset($_GET["validaciones"]) && !empty($_GET["validaciones"])) {
    ...
    if ($strTipoValidacion == "insert") {
        ...
        $rs2 = pg_query("SELECT CORRPAQ FROM CG000001");
        ...
        foreach ($contactList as $key => $contactData) {
            ...
            $rs2 = pg_query("SELECT numtrans, numenv, numempre, codiempr FROM gc000001 WHERE TRIM(codiclie) = '{$contactData[1]}'");
            if ($row = pg_fetch_row($rs2)) {
                $idRow1 = trim($row[0]);
                $idRow2 = trim($row[1]);
                $idRow3 = trim($row[2]);
            ...
            $var_consulta = "INSERT INTO GM000001 ( CODICLIE, FGESTION, HORA, TIPOLOGI, PLACE, CONCLUSI, RTESTADO, SUBCONCL, OBSERVAC , NUMTRANS, NUMENV, NUMEMPRE, CODIEMPR, OWNER, CPLACE,CRTESTAD,CTIPOLOG,CCONCLUS,CSUBCONC) VALUES('{$contactData[1]}','{$contactData[2]}', '{$contactData[3]}','{$contactData[4]}','{$contactData[5]}','{$contactData[6]}','{$contactData[7]}','{$contactData[8]}','{$contactData[9]}', $NUMTRANS, '$NUMENV', '$NUMEMPRE', '$CODIEMPR','OPERACIONES',' ',' ',' ',' ',' ');";
            ...
            // print 'var_consulta   ' . $var_consulta . '<br>   <br>    ';
        }
        echo $val;
